...
const ReviewDialog = forwardRef<HTMLDialogElement, { draft: Draft; mainnet: boolean }>(function ReviewDialog(
  ...
  async function resumePool() {
    if (!token || !account || chainId === null) return;
    setHood("working");
    try {
      const liq = await addLiquidity({
        chainId,
  ...
  return (
    <dialog ref={ref} aria-label="Review your launch">
      ...
      {chainId === null ? (
        <p role="alert">Unsupported chain.</p>
      ) : (
...
NOTE (final cross-plan fix, in repo): the whole Hood block above (WalletButton through Resume button) is wrapped in `{!isPump && (<>...</>)}` where `const isPump = draft.route === "pumpfun" && String(draft.chainId).startsWith("solana")`, so Solana drafts never see Hood actions. `resumePool()` also calls `await ensureChain(chainId)` first (after its guard) to avoid resuming on a stale chain.
...
- `lib/launcher-evm.ts`: `ETH_MIN_BPS = 9800` (2% ETH slippage tolerance) via
  `calcEthMin()`; `TX_DEADLINE_SECS = 600` (10-min deadline). `addLiquidity`
  keeps exact token minimum, ETH minimum is now 98%. `launchOneTx` uses the
  same 98% ETH minimum instead of `0n`.
- `components/ReviewDialog.tsx`: shows the protection note
  (2% slippage, 10-min deadline). 2-step resume flow unchanged.
- Ruling: mainnet `KentirLauncher` deploy deferred to a user-run ceremony
  (needs funded deployer + Blockscout verification). Code paths for 1-tx are
  ready; `launcher_unavailable` still guards until the address is set.
