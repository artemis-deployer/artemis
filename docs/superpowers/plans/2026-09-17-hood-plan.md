...
- `contracts/ArtemisToken.sol` — ERC20 template source of truth.
...
**Files:**
- Create: `contracts/ArtemisToken.sol`, `scripts/compile-token.mjs`, `lib/token-artifact.ts` (generated), `tests/token-artifact.test.ts`
...
- [ ] **Step 2: Write contracts/ArtemisToken.sol**
...
const source = readFileSync(new URL("../contracts/ArtemisToken.sol", import.meta.url), "utf8");
const input = {
  language: "Solidity",
  sources: { "ArtemisToken.sol": { content: source } },
  settings: {
...
const contract = output.contracts["ArtemisToken.sol"]["ArtemisToken"];
...
3. Expected: token address appears, state `stub`, rehearsal note shows, receipt saved (check Application → Local Storage `artemis.receipts.v1`).
...
- Ruling: mainnet `ArtemisLauncher` deploy deferred to a user-run ceremony
